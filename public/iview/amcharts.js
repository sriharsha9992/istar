...
AmCharts.AmChart = AmCharts.Class({
	...
	drawChart: function () {
		...
		this.realWidth = b = this.updateWidth();
		this.realHeight = d = this.updateHeight();
		this.redrawLabels();
		this.drawTitles()
	},
	drawTitles: function () {
	...
	write: function (a) {
		...
		if (e) switch (e.position) {
			...
			case "right":
				...
				break;
			case "left":
				h = document.createElement("div"), i = h.style, i.position = "relative", i.width = a.style.width, i.height = a.style.height,
				a.appendChild(h), f.position = "absolute", g.position = "relative", h.appendChild(b), h.appendChild(d)
...
AmCharts.AmRectangularChart = AmCharts.Class({
	...
	updatePlotArea: function () {
		var a = this.updateWidth() - 1,
			b = this.updateHeight() - 1,
			d = this.container;
		this.realWidth = a;
		this.realWidth = b;
	...
	updateObjectSize: function (a) {
		...
		a.height = this.plotAreaHeight * this.heightMultiplier;
...
AmCharts.RecItem = AmCharts.Class({
	construct: function (a, b, d, e, f, g, h, i, j) {
		...
		var R = E * Math.PI / 180,
			...
			da = ha = 0;
		"V" == S && (E = 0);
		if (J) var T = AmCharts.text(r, d, m, k, l, O, i),
			da = T.getBBox().width;
		if ("H" == S) {
			if (0 <= b && b <= $ + 1 && (0 < t && (0 < s && b + j <= $ + 1) && (ba = AmCharts.line(r, [b + j, b + j], [0, t], n, s, x), p.push(ba)), 0 < u && (O = AmCharts.line(r, [b, b + N, b + N], [fa, fa + F, F], z, u, x, y), p.push(O))), w = 0, D = b, h && 90 == E && (D -= l), !1 == e ? (O = "start", w = "bottom" == M ? L ? w + t : w - t : L ? w - t : w + t, D += 3, f && (D += f / 2, O = "middle"), 0 < E && (O = "middle")) : O = "middle", 1 == W && (0 < C && !h && Q < $) && (e = AmCharts.fitToBounds(b, 0, $), Q = AmCharts.fitToBounds(Q, 0, $), ha = e - Q, 0 < ha && (fill = AmCharts.rect(r, ha, a.height, v, C), fill.translate(e - ha + N, F), p.push(fill))), "bottom" == M ? (w += fa + l / 2 + aa, L ? 0 < E ? (w = fa - da / 2 * Math.sin(R) - t - 3, D += da / 2 * Math.cos(R)) : w -= t + l + 3 + 3 : 0 < E ? (w = fa + da / 2 * Math.sin(R) + t + 3, D -= da / 2 * Math.cos(R)) : w += t + q + 3 + 3) : (w += F + l / 2 - aa, D += N, L ? 0 < E ? (w = da / 2 * Math.sin(R) + t + 3, D -= da / 2 * Math.cos(R)) : w += t + 3 : 0 < E ? (w = -(da / 2) * Math.sin(R) - t - 6, D += da / 2 * Math.cos(R)) : w -= t + l + 3 + q + 3), "bottom" == M ? ha = (L ? fa - t - 1 : fa + q - 1) + aa : (ga = N, ha = (L ? F : F - t - q + 1) - aa), g && (D += g), F = D, 0 < E && (F += da / 2 * Math.cos(R)), T && (M = 0, L && (M = da * Math.cos(R)), F + M > $ + 1 || 0 > F)) T.remove(), T = null
		} else {
			0 <= b && b <= fa + 1 && (0 < t && (0 < s && b + j <= fa + 1) && (ba = AmCharts.line(r, [0, t], [b + j, b + j], n, s, x), p.push(ba)), 0 < u && (O = AmCharts.line(r, [0, N, $ + N], [b, b + F, b + F], z, u, x, y), p.push(O)));
			O = "end";
			if (!0 == L && "left" == M || !1 == L && "right" == M) O = "start";
			w = b - l / 2;
			1 == W && (0 < C && !h) && (e = AmCharts.fitToBounds(b, 0, fa), Q = AmCharts.fitToBounds(Q, 0, fa), R = e - Q, fill = AmCharts.polygon(r, [0, a.width, a.width, 0], [0, 0, R, R], v, C), fill.translate(N, e - R + F), p.push(fill));
			w += l / 2;
			"right" == M ? (D += N + $ + aa, w += F, L ? (D -= t + 4, g || (w -= l / 2 + 3)) : (D += t + 4 + q, w -= 2)) : L ? (D += t + 4 - aa, g || (w -= l / 2 + 3), h && (D += N, w += F)) : (D += -t - q - 4 - 2 - aa, w -= 2);
			ba && ("right" == M ? (ga += N + aa + $, ha += F, ga = L ? ga - q : ga + q) : (ga -= aa, L || (ga -= t + q)));
			g && (w += g);
			L = -3;
			"right" == M && (L += F);
			if (T && (w > fa + 1 || w < L)) T.remove(), T = null
		}
		ba && ba.translate(ga, ha);
		!1 == a.visible && (ba && ba.remove(), T && (T.remove(), T = null));
		T && (T.attr({
			"text-anchor": O
		}), T.translate(D, w), 0 != E && T.rotate(-E), a.allLabels.push(T), " " != d && (this.label = T));
		a.counter = 0 == W ? 1 : 0;
		a.previousCoord = b;
...
AmCharts.RadItem = AmCharts.Class({
	construct: function (a, b, d, e, f, g, h) {
		...
		if (z) {
			...
			L.translate(g + (p + 3) * W, b);
			e.push(L);
			this.label = L;
			C = AmCharts.line(l, [g, g + p * W], [b, b], m, r, n);
			e.push(C)
		}
		b = a.y - b;
		if ("polygons" == v) {
			for (var N = [], F = [], S = a.data.length, d = 0; d < S; d++) i = 180 - 360 / S * d, N.push(b * Math.sin(i / 180 * Math.PI)), F.push(b * Math.cos(i / 180 * Math.PI));
			N.push(N[0]);
			F.push(F[0]);
		...
		if (1 == f && 0 < x && !h) {
			h = a.previousCoord;
			if ("polygons" == v) {
				for (d = S; 0 <= d; d--) i = 180 - 360 / S * d, N.push(h * Math.sin(i / 180 * Math.PI)), F.push(h * Math.cos(i / 180 * Math.PI));
...
AmCharts.AmGraph = AmCharts.Class({
	...
	createSerialGraph: function () {
		...
		if (!W && ("line" == g || "smoothedLine" == g || "step" == g)) if (1 == d.length && ("step" != g && "none" == this.bullet) && (this.bullet = "round", this.resetBullet = !0), Da || void 0 != va) {
			var wa = F;
			wa > f.max && (wa = f.max);
			wa < f.min && (wa = f.min);
			f.logarithmic && (wa = f.minReal);
			var ra = f.getCoordinate(wa),
				lb = f.getCoordinate(f.max);
			l ? (da = j, T = Math.abs(lb - ra), Ta = j, Ua = Math.abs(Na - ra), Ma = Sa = 0, f.reversed ? (Ka = 0, La = ra) : (Ka = ra, La = 0)) : (T = i, da = Math.abs(lb - ra), Ua = i, Ta = Math.abs(Na - ra), La = Ka = 0, f.reversed ? (Ma = k, Sa = ra) : Ma = ra + 1)
		}
		var sa = Math.round;
		this.pmx = sa(Ka);
		this.pmy = sa(Sa);
		...
		this.nmh = sa(Ta);
		this.nmw = sa(Ua);
		h = "column" == g ? (R * h - O * (m - 1)) / m : R * h;
		1 > h && (h = 1);
		var K;
		if ("line" == g || "step" == g || "smoothedLine" == g) {
			if (0 < J) for (K = J - 1; - 1 < K; K--) if (D = d[K], w = D.axes[f.id].graphs[a], ga = w.values.value) {
				J = K;
				break
			}
			if (E < d.length - 1) for (K = E + 1; K < d.length; K++) if (D = d[K], w = D.axes[f.id].graphs[a], ga = w.values.value) {
				E = K;
				break
			}
		}
		...
		var ka = [],
			la = [],
			Va = !1;
		if ("line" == g || "step" == g || "smoothedLine" == g) if (this.stackable && "regular" == v || "100%" == v) Va = !0;
		for (K = J; K <= E; K++) {
			D = d[K];
			...
			var pa = w.values;
			f.recalculateToPercents && (pa = w.percents);
			if (pa) {
				P = !this.stackable || "none" == v || "3d" == v ? pa.value : pa.close;
				if ("candlestick" == g || "ohlc" == g) var P = pa.close,
					Wa = pa.low,
					xa = f.getCoordinate(Wa),
					Xa = pa.high,
					ya = f.getCoordinate(Xa);
				var ia = pa.open,
					A = f.getCoordinate(P);
				isNaN(ia) || (U = f.getCoordinate(ia));
				...
				var ma = D.x[L.id],
					za = Math.round(R / 2),
					Ya = za;
				"start" == this.pointPosition && (ma -= R / 2, za = 0, Ya = R);
				W || (w.x = ma); - 1E5 > ma && (ma = -1E5);
				ma > i + 1E5 && (ma = i + 1E5);
				l ? (B = A, X = U, U = A = ma, isNaN(ia) && (X = N), Oa = xa, Pa = ya) : (X = B = ma, isNaN(ia) && (U = N));
				switch (g) {
					case "line":
						isNaN(P) ? p || (this.drawLineGraph(q, n, ka, la), q = [], n = [], ka = [], la = []) : (w.isNegative = P < F ? !0 : !1, q.push(B), n.push(A), V = B, Y = A, na = B, oa = A, Va && (ka.push(X),
						la.push(U)));
						break;
					case "smoothedLine":
						isNaN(P) ? p || (this.drawSmoothedGraph(q, n, ka, la), q = [], n = [], ka = [], la = []) : (w.isNegative = P < F ? !0 : !1, q.push(B), n.push(A), V = B, Y = A, na = B, oa = A, Va && (ka.push(X), la.push(U)));
						break;
					case "step":
						isNaN(P) ? p || (t = NaN, this.drawLineGraph(q, n, ka, la), q = [], n = [], ka = [], la = []) : (w.isNegative = P < F ? !0 : !1, l ? (isNaN(s) || (q.push(s), n.push(A - za)), n.push(A - za), q.push(B), n.push(A + Ya), q.push(B)) : (isNaN(t) || (n.push(t), q.push(B - za)), q.push(B - za), n.push(A), q.push(B + Ya), n.push(A)), s = B, t = A, V = B, Y = A, na = B, oa = A);
						break;
					case "column":
						var Aa = ea;
						void 0 != w.lineColor && (Aa = w.lineColor);
						if (!isNaN(P)) {
							P < F ? (w.isNegative = !0, Da && (ta = Da), void 0 != va && (ea = va)) : w.isNegative = !1;
							var mb = f.min,
								nb = f.max;
							if (!(P < mb && (ia < mb || void 0 == ia) || P > nb && ia > nb)) if (l) {
								if ("3d" == v) var H = A - 0.5 * (h + O) + O / 2 + y * S,
									I = X + z * S;
								else H = A - (m / 2 - S) * (h + O) + O / 2, I = X;
								var G = h,
									V = B,
									Y = H + h / 2,
									na = B,
								...
								var Z = B - X,
									Ab = I,
									I = AmCharts.fitToBounds(I, 0, i),
									Z = Z + (Ab - I),
									Z = AmCharts.fitToBounds(Z, - I, i - I + z * S);
								if (H < j && 0 < G && (ja = new AmCharts.Cuboid(e,
								Z, G, z, y, ta, Ga, M, Aa, Q, ba, r, l), "bottom" != C)) if (C = f.reversed ? "left" : "right", 0 > P) C = f.reversed ? "right" : "left";
								else if ("regular" == v || "100%" == v) V += this.dx
							} else {
								"3d" == v ? (I = B - 0.5 * (h + O) + O / 2 + z * S, H = U + y * S) : (I = B - (m / 2 - S) * (h + O) + O / 2, H = U);
								G = h;
								V = I + h / 2;
								Y = A;
								na = I + h / 2;
								...
								var Z = A - U,
									Bb = H,
									H = AmCharts.fitToBounds(H, 0, j),
									Z = Z + (Bb - H),
									Z = AmCharts.fitToBounds(Z, - H + y * S, j - H);
								if (I < i + S * z && 0 < G) if (ja = new AmCharts.Cuboid(e, G, Z, z, y, ta, Ga, M, Aa, this.lineAlpha, ba, r, l), 0 > P && "middle" != C) C = "bottom";
								else if ("regular" == v || "100%" == v) Y += this.dy
							}
							if (ja) {
								var qa = ja.set;
								qa.translate(I, H);
								this.columnsSet.push(qa);
								w.url && qa.setAttr("cursor", "pointer");
								if (!W) {
									"none" == v && (x = l ? (this.end + 1 - K) * u - b : u * K + b);
									"3d" == v && (l ? (x = (u - b) * (this.end + 1 - K), V += z * this.columnIndex, na += z * this.columnIndex, w.y += z * this.columnIndex) : (x = (u - b) * (K + 1), V += 3, Y += y * this.columnIndex + 7, oa += y * this.columnIndex, w.y += y * this.columnIndex));
									if ("regular" == v || "100%" == v) C = "middle", x = l ? 0 < pa.value ? (this.end + 1 - K) * u + b : (this.end + 1 - K) * u - b : 0 < pa.value ? u * K + b : u * K - b;
									...
									this.animateColumns(ja, K, B, X, A, U);
									this.addListeners(qa, w)
								}
								w.columnSprite = qa
							}
						}
						break;
					case "candlestick":
						if (!isNaN(ia) && !isNaN(Xa) && !isNaN(Wa) && !isNaN(P)) {
							var Qa, Za;
							P < ia && (w.isNegative = !0, Da && (ta = Da), Fa && (Ga = Fa), void 0 != va && (ea = va));
							Aa = ea;
							if (l) {
								if (H = A - h / 2, I = X, G = h, H + G > j && (G = j - H), 0 > H && (G += H, H = 0), H < j && 0 < G) {
									var $a, ab;
									P > ia ? ($a = [B, Pa], ab = [X, Oa]) : ($a = [X, Pa], ab = [B, Oa]);
									A < j && 0 < A && (Qa = AmCharts.line(e, $a, [A, A], ea, Q, M), Za = AmCharts.line(e, ab, [A, A], ea, Q, M));
									Z = B - X;
									ja = new AmCharts.Cuboid(e, Z, G, z, y, ta, Ea, M, Aa, Q, ba, r, l)
								}
							} else if (I = B - h / 2, H = U + M / 2, G = h, I + G > i && (G = i - I), 0 > I && (G += I, I = 0), Z = A - U, I < i && 0 < G) {
								var ja = new AmCharts.Cuboid(e, G, Z, z, y, ta, Ga, M, Aa, Q, ba, r, l),
									bb, cb;
								P > ia ? (bb = [A, ya], cb = [U, xa]) : (bb = [U, ya], cb = [A, xa]);
								B < i && 0 < B && (Qa = AmCharts.line(e, [B, B], bb, ea, Q, M), Za = AmCharts.line(e, [B, B], cb, ea, Q, M))
							}
							ja && (qa = ja.set, aa.push(qa), qa.translate(I, H), w.url && qa.setAttr("cursor", "pointer"), Qa && (aa.push(Qa), aa.push(Za)), V = B, Y = A, na = B, oa = A, W || (w.x = l ? H + G / 2 : I + G / 2, this.animateColumns(ja,
							K, B, X, A, U), this.addListeners(qa, w)))
						}
						break;
					case "ohlc":
						if (!isNaN(ia) && !isNaN(Xa) && !isNaN(Wa) && !isNaN(P)) {
							P < ia && (w.isNegative = !0, void 0 != va && (ea = va));
							var db, eb, fb;
							if (l) {
								var gb = A - h / 2,
								...
								eb = AmCharts.line(e, [X, X], [gb, ob], ea, Q, M, $);
								0 < A && A < j && (db = AmCharts.line(e, [Oa, Pa], [A, A], ea, Q, M, $));
								fb = AmCharts.line(e, [B, B], [ob, hb], ea, Q, M, $)
							} else {
								var ib = B - h / 2,
									ib = AmCharts.fitToBounds(ib, 0, i),
									pb = AmCharts.fitToBounds(B,
									0, i),
									jb = B + h / 2,
									jb = AmCharts.fitToBounds(jb, 0, i);
								eb = AmCharts.line(e, [ib, pb], [U, U], ea, Q, M, $);
								0 < B && B < i && (db = AmCharts.line(e, [B, B], [xa, ya], ea, Q, M, $));
								fb = AmCharts.line(e, [pb, jb], [A, A], ea, Q, M, $)
							}
							aa.push(eb);
							aa.push(db);
							aa.push(fb);
				...
				if (!W && !isNaN(P)) {
					...
					if (this.end - this.start <= qb || 0 == qb) {
						var Ba = this.createBullet(w, na, oa, K);
						Ba || (Ba = 0);
						...
						if (rb) {
							...
							switch (C) {
								...
								case "right":
									...
									break;
								case "bottom":
									l && "column" == g ? (V = N, 0 > P ? (ua = -6, ca.attr({
										"text-anchor": "end"
									})) : (ua = 6, ca.attr({
										"text-anchor": "start"
									...
									break;
								case "middle":
									"column" == g && (l ? (ua = -(B - X) / 2 - z, 0 > Z && (ua += z), Math.abs(B - X) < Ra && !this.showAllValueLabels && (ca.remove(), ca = null)) : (Ca = -(A - U) / 2, 0 > Z && (Ca -= y), Math.abs(A - U) < kb && !this.showAllValueLabels && (ca.remove(), ca = null)))
							}
							if (ca) if (!isNaN(Y) && !isNaN(V)) if (V += ua,
							Y += Ca, ca.translate(V, Y), l) {
								if (0 > Y || Y > j) ca.remove(), ca = null
							} else {
								var tb = 0;
								"3d" == v && (tb = z * S);
								if (0 > V || V > i + tb) ca.remove(), ca = null
		...
		if ("line" == g || "step" == g || "smoothedLine" == g) "smoothedLine" == g ? this.drawSmoothedGraph(q, n, ka, la) : this.drawLineGraph(q, n, ka, la), W || this.launchAnimation()
	},
...
AmCharts.ChartCursor = AmCharts.Class({
	...
	createCursor: function () {
		var a = this.chart,
			...
			n = d.tickLength;
		h.pointerWidth = n;
		a ? (i = [0, p, p + k], j = [0, 0, l]) : (i = [k, 0, 0], j = [l, 0, q - 1]);
		...
		if (this.enabled) {
			...
			if (d.mouseIsOver || v || C || this.forceShow) if (this.forceShow = !1, C) {
				...
				0 != e && (k = {
					type: "zoomed",
					target: this
				}, k.chart = this.chart, g.parseDates && !g.equalSpacing ? (d + e > x && (e = x - d), v + e < u && (e = u - v), k.start = v + e, k.end = d + e, this.fire(k.type, k)) : h + e >= this.data.length || 0 > j + e || (k.start = j + e, k.end = h + e, this.fire(k.type, k)))
			} else {
				...
				u = !0;
				v && (u = !1);
				this.categoryBalloonEnabled && u ? (q ? (n && ("right" == y ? t.setBounds(j, k + i, j + l + h, k + p + i) : t.setBounds(j, k + i, j + l + h, k + p)), "right" == y ? n ? t.setPosition(j + l + h, k + p + i) : t.setPosition(j + l + h + E, k + p + i) : n ? t.setPosition(j, k + p) : t.setPosition(j - E, k + p)) : "top" == y ? n ? t.setPosition(j + p + h, k + i) : t.setPosition(j + p + h, k + i - E + 1) : n ? t.setPosition(j + p, k + m) : t.setPosition(j + p, k + m + E - 1), g.parseDates ? (g = AmCharts.formatDate(r.category, this.categoryBalloonDateFormat), - 1 != g.indexOf("fff") && (g = AmCharts.formatMilliseconds(g, r.category)), t.showBalloon(g)) : t.showBalloon(r.category)) : t.hide();
				J && this.bulletsEnabled && this.showBullets();
				this.destroyValueBalloons();
				if (J && this.valueBalloonsEnabled && u && d.balloon.enabled) {
					this.valueBalloons = g = [];
					if (this.oneBalloonOnly) for (var i = Infinity, W, u = 0; u < J.length; u++) s = J[u], s.showBalloon && (!s.hidden && s.balloonText) && (t = r.axes[s.valueAxis.id].graphs[s.id], x = t.y, isNaN(x) || (q ? Math.abs(e - x) < i && (i = Math.abs(e - x), W = s) : Math.abs(f - x) < i && (i = Math.abs(f - x), W = s)));
					for (u = 0; u < J.length; u++) if (s = J[u], !(this.oneBalloonOnly && s != W) && (s.showBalloon && !s.hidden && s.balloonText) && (t = r.axes[s.valueAxis.id].graphs[s.id],
					x = t.y, !isNaN(x))) {
						p = t.x;
						n = !0;
						if (q) {
				...
				k.target = this, k.chart = this.chart, k.zooming = v, k.position = q ? f : e, k.target = this, d.fire("changed", k), this.fire("changed", k), this.skipZoomDispatch = !1) : (this.skipZoomDispatch = !0, d.updateLegendValues(a));
...
AmCharts.wedge = function (a, b, d, e, f, g, h, i, j, k, l) {
	...
	if (0 < j) {
		...
		e.push(c);
		var E = a.path(" M" + q + "," + s + " L" + q + "," + (s + j) + " L" + t + "," + (u + j) + " L" + t + "," + u + " L" + q + "," + s + " Z", void 0, void 0, "1000,1000").attr(v),
			j = a.path(" M" + x + "," + z + " L" + x + "," + (z + j) + " L" + y + "," + (n + j) + " L" + y + "," + n + " L" + x + "," + z + " Z", void 0, void 0, "1000,1000").attr(v);
		e.push(E);
		e.push(j)
	}
